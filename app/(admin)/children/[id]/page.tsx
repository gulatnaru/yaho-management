...
export default async function ChildDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [child, reservations, safetyInfo, consent] = await Promise.all([getChildDetail(id), listReservationsByChild(id), getChildSafetyInfo(id), getChildConsentSummary(id)]);
  if (!child) notFound();
  ...
  return (
    <section className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>동의 현황 및 이력</CardTitle></CardHeader><CardContent className="space-y-4"><div className="grid gap-2 sm:grid-cols-2">{CONSENT_TYPES.map((type) => { const record = consent.current.get(type); return <div className="flex items-center justify-between rounded-md border p-3 text-sm" key={type}><span>{CONSENT_LABEL[type]}</span><Badge variant={record?.action === "AGREED" ? "success" : "secondary"}>{record?.action === "AGREED" ? "동의" : record ? "철회" : "미기록"}</Badge></div>; })}</div><ConsentForm childId={id} />{consent.history.length > 0 ? <details><summary className="cursor-pointer text-sm font-medium">전체 이력 보기 ({consent.history.length})</summary><ul className="mt-2 space-y-1 text-xs text-slate-600">{consent.history.map((record) => <li key={record.id}>{CONSENT_LABEL[record.consentType as ConsentType]} · {record.action === "AGREED" ? "동의" : "철회"} · {record.recordedAt.toISOString()} · {record.recordedBy?.name ?? "알 수 없음"}</li>)}</ul></details> : null}</CardContent></Card>
      <Card><CardHeader><CardTitle>예약 이력</CardTitle></CardHeader><CardContent>{reservations.length === 0 ? <p className="text-sm text-slate-500">예약 이력이 없습니다.</p> : <ul className="space-y-2">{reservations.map((reservation) => { const status = getReservationDisplayStatus(reservation, reservation.classSchedule); return <li className="flex items-center justify-between gap-3 text-sm" key={reservation.id}><Link className="font-medium hover:underline" href={`/classes/${reservation.classSchedule.id}`}>{reservation.classSchedule.program.name}<span className="block text-xs text-slate-500">{formatKstDateTimeRange(reservation.classSchedule.startsAt, reservation.classSchedule.endsAt)}</span></Link><span className="flex items-center gap-3"><Badge variant={STATUS_VARIANT[status]}>{STATUS_LABEL[status]}</Badge>{status === "RESERVED" ? <Link className="text-red-600 hover:underline" href={`/reservations/${reservation.id}/cancel`}>예약 취소</Link> : null}</span></li>; })}</ul>}</CardContent></Card>
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2"><PlaceholderSection description="Phase 7에서 제공 예정" title="친구관계" /><PlaceholderSection description="Phase 7에서 제공 예정" title="형제·자매관계" /><PlaceholderSection description="Phase 8에서 제공 예정" title="결제 이력" /><PlaceholderSection description="Phase 8에서 제공 예정" title="환불 이력" /></div>
    </section>
  );
}
