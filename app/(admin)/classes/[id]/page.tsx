...
export default async function ClassDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return (
    <section className="space-y-6">
      ...
      <Card><CardHeader><CardTitle>참여 아이 · 안전 정보 · 출결</CardTitle></CardHeader><CardContent>{reservations.length === 0 ? <p className="text-sm text-slate-500">예약된 아이가 없습니다.</p> : <ul className="space-y-4">{reservations.map((reservation) => { const status = getReservationDisplayStatus(reservation, classDetail); const safety = reservation.child.safetyInfo; return <li className="space-y-3 rounded-md border p-3" key={reservation.id}><div className="flex flex-wrap items-center justify-between gap-2"><Link className="font-medium hover:underline" href={`/children/${reservation.child.id}`}>{reservation.child.name}</Link><span className="flex items-center gap-3"><Badge variant={RESERVATION_VARIANT[status]}>{RESERVATION_LABEL[status]}</Badge>{status === "RESERVED" ? <Link className="text-red-600 hover:underline" href={`/reservations/${reservation.id}/cancel`}>예약 취소</Link> : null}</span></div><div className="grid gap-2 text-xs text-slate-600 sm:grid-cols-3"><span>알레르기: {safety?.allergies || "없음"}</span><span>응급 유의사항: {safety?.emergencyNotes || "없음"}</span><span>비상연락처: {safety?.emergencyContactName || "미입력"} {safety?.emergencyContactPhone || ""}</span></div>{reservation.attendanceRecordedAt ? <p className="text-xs text-slate-500">출결 기록: {reservation.attendanceRecordedBy?.name ?? "알 수 없음"} · {reservation.attendanceRecordedAt.toISOString()}</p> : null}{classDetail.status !== "CANCELLED" && reservation.status !== "CANCELLED" ? <AttendanceForm current={reservation.attendance} reservationId={reservation.id} /> : null}</li>; })}</ul>}</CardContent></Card>
    </section>
  );
}
